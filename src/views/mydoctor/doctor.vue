	...
	export default{
	  ...
	  data () {
		...
		methods: {
			...
			// 跳转到留言页面
			toMsgUrl () {
				window.location.href='http://wx.leerhuo.com/html/pay/vue_msg_v.html?openID='+ openID + '&drID=' + this.ids + '&drName=' + this.drName + '&drTitle=' + this.drTitle + '&toHome=doctor';
			},
			// 跳转到电话预约
			toCallUrl () {
				let _self = this
				if(_self.isPhone == true){
					window.location.href='http://wx.leerhuo.com/html/pay/vue_call_v.html?id='+this.ids;
				}else{
